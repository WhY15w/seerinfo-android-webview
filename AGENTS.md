...
- Release is automated by pushing a tag matching `v*` (e.g. `v1.26`); `.github/workflows/android-release.yml` builds, **signs** (secrets `SIGNING_KEY`/`KEY_ALIAS`/`KEYSTORE_PASSWORD`/`KEY_PASSWORD`), and publishes a GitHub Release. Local `assembleRelease` is unsigned and uses `isMinifyEnabled = false`.

## App-specific gotchas

- JS bridge: `WebAppInterface` is injected as the JS object `Android` (e.g. `window.Android.clearAllCache()`, `getAppVersion()`, `showToast()`). New web-callable methods require the `@JavascriptInterface` annotation; WebView mutations must run on the main thread (`Dispatchers.Main`), as existing methods do.
